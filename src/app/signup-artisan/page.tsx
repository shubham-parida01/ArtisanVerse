...
export default function SignupArtisanPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/signup-artisan', {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        toast({ title: "Welcome, Artisan!", description: "Your account has been created. Please log in." });
        router.push('/login-artisan');
      }
    } catch (error) {
    ...
    <div className="container mx-auto max-w-sm px-4 py-12 md:px-6">
      <Card>
        <CardHeader className="text-center">
          <CardTitle className="text-3xl font-bold font-headline">Create an Artisan Account</CardTitle>
          <CardDescription className="text-lg">Start selling your crafts on ArtisanVerse.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
                <Label htmlFor="name">Full Name or Brand</Label>
                <Input id="name" name="name" placeholder="e.g., Elena Vance" required disabled={isLoading} />
            ...
            <div className="text-center text-sm">
                Already an artisan?{' '}
                <Link href="/login-artisan" className="font-medium text-primary hover:underline">
                    Log in
                </Link>
            </div>
          </form>
